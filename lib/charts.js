...
// Constants
const ROUND_CHART_TYPES = new Set(['pie', 'doughnut', 'polarArea', 'outlabeledPie', 'outlabeledDoughnut']);
...
const DEFAULT_FONT_FAMILY = process.env.CHART_FONT_FAMILY || 'Gibson, Tahoma, Arial, sans-serif';
...
async function getRenderer(width, height, version, format) {
  ...
  if (!rendererCache[key]) {
    ...
    if (DEFAULT_FONT_FAMILY && DEFAULT_FONT_FAMILY != '') {
      rendererCache[key] = new CanvasRenderService(width, height, ChartJS => {
        ChartJS.defaults.global.defaultFontFamily = `${DEFAULT_FONT_FAMILY}`;
      }, format, () => Chart)
    } else {
...
function patternDraw(shapeType, backgroundColor, patternColor, requestedSize) {
  ...
  global.document = {
    createElement: () => canvas.createCanvas(size, size),
  };
...
async function renderChartJs(
  ...
  if (chart.type === 'bar' || chart.type === 'horizontalBar' || chart.type === 'line' || chart.type === 'scatter' || chart.type === 'bubble') {
    if (!chart.options.scales) {
